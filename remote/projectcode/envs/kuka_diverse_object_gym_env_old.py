...
class KukaDiverseObjectEnv(KukaGymEnv):
  ...
  def set_objects_at(self, urdfList):
    """Environment reset called at the beginning of an episode.
    """
    # Set the camera settings.
    look = [0.23, 0.2, 0.54]
  ...
  def update_ego_view(self):
    """
    Set camera view to pose of kuka's end-effector: therefore we use pybullet's computeViewMatrix function,
    which takes (cameraEyePosition, cameraTargetPosition, cameraUpVector) as arguments.
    In ego-view:
        - cameraEyePosition = position of the kuka's endeffector
        - cameraTargetPosition = position of the kuka's endeffector - dz (looking straight downward)
        - cameraUpVector = [sin(endEffectorAngle), cos(endEffectorAngle), 0.0] (view rotating with hand)
    """
