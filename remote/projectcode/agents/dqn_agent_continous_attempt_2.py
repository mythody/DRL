...
class DQNAgent(object):
        ...
        if (self.env._isDiscrete):
            ...
            if sample > self.eps_threshold:
                with torch.no_grad():
                    ...
                    action_mean = self.policy_net(state) #self(observation)
                    ...
                    action = action_distribution.sample().view(self.n_actions)  # don't bother with rsample
                    return action
            else:
                #return torch.tensor([[random.randrange(self.n_actions)]], device=self.device, dtype=torch.long)
                #self.uniform_distribution = 2 * torch.rand(self.n_actions) - 1
                action = torch.distributions.uniform.Uniform(-1,+1).sample([self.n_actions]).view(self.n_actions)
                return action
    ...
    def optimize_model(self):
        ...
        #
        # For continuous action space: sample 16 actions uniformly at random and pick the one with the largest Q-value
        largest_q_value = 0
        for i in range(16):
            action_i = torch.distributions.uniform.Uniform(-1, +1).sample([self.n_actions]).view(self.n_actions)
